...
impl Solution {
    ...
    pub fn backspace_compare(s: String, t: String) -> bool {
        ...
        loop {
            ...
            if s_char.is_none() && t_char.is_none() {
                return true;
            }
            if s_char != t_char {
                return false;
    ...
    pub fn find_max_length(nums: Vec<i32>) -> i32 {
        ...
        for (i, n) in nums.into_iter().enumerate() {
            ...
            if sums.contains_key(&sum) {
                max_len = max_len.max(i as i32 - sums.get(&sum).unwrap().clone())
            } else {
    ...
    pub fn find_max_length_v2(nums: Vec<i32>) -> i32 {
        ...
        for (i, n) in nums.into_iter().enumerate() {
            ...
            if sum == 0 {
                ...
                if sums.contains_key(&sum) {
                    max_len = max_len.max(i - sums.get(&sum).unwrap().clone())
                } else {
                    sums.insert(sum, i);
                }
            }
    ...
    pub fn string_shift(s: String, shift: Vec<Vec<i32>>) -> String {
        ...
        let sum = shift.into_iter().fold(0, |sum, val| {
            sum + if val[0] == 0 { -val[1] } else { val[1] }
        }) % l as i32;
        if sum == 0 {
            return s;
        }
    ...
    pub fn check_valid_string(s: String) -> bool {
        ...
        if par_stack.len() <= ast_stack.len() {
            while !par_stack.is_empty() {
                if par_stack.pop() > ast_stack.pop() {
                    return false;
                }
            }
            return true;
    ...
    pub fn num_islands_v2(mut grid: Vec<Vec<char>>) -> i32 {
        ...
        fn visit_cell((x, y): (i32, i32), &(w, h): &(i32, i32), grid: &mut Vec<Vec<char>>) {
            ...
            let neighbor_cells = [(x + 1, y), (x, y + 1), (x - 1, y), (x, y - 1)];
            for &(ax, ay) in &neighbor_cells {
                if ax >= 0 && ax < w && ay >= 0 && ay < h && grid[ay as usize][ax as usize] == '1' {
    ...
    pub fn bst_from_preorder_v2(preorder: Vec<i32>) -> Option<Rc<RefCell<TreeNode>>> {
        fn build_bst(
            ...
            if arr[i] < curr_node.val {
                ...
                i = build_bst(
                    arr,
                    i + 1,
                    RefCell::borrow_mut(curr_node.left.as_ref().unwrap()),
                    left,
                    Some(curr_node.val - 1),
                );
                if i == arr.len()
                    || (left.is_some() && arr[i] < *left.as_ref().unwrap())
                    || (right.is_some() && arr[i] > *right.as_ref().unwrap())
            ...
            i = build_bst(
                arr,
                i + 1,
                RefCell::borrow_mut(curr_node.right.as_ref().unwrap()),
                Some(curr_node.val + 1),
                right,
            );
            return i;
        ...
        build_bst(
            &preorder,
            1,
            RefCell::borrow_mut(root.as_ref().unwrap()),
            None,
            None,
        );
        root
    ...
    pub fn longest_common_subsequence(text_1: String, text_2: String) -> i32 {
        if text_1.is_empty() || text_2.is_empty() {
            return 0;
        }
        ...
        for r in 1..rows {
            for c in 1..cols {
                if chars_1[r - 1] == chars_2[c - 1] {
                    ...
                    table[r][c] = table[r - 1][c].max(table[r][c - 1]);
                }
            }
        }
        ...
        for r in 1..rows {
            for c in 1..cols {
                if matrix[r - 1][c - 1] == '0' {
                    ...
                    table[r][c] = 1 + table[r - 1][c - 1].min(table[r - 1][c].min(table[r][c - 1]));
                    max_sz = max_sz.max(table[r][c]);
                }
            }
        }
        max_sz * max_sz
        ...
        for r in 1..rows {
            for c in 1..cols {
                if matrix[r - 1][c - 1] == '0' {
                    ...
                    let tmp = prev_row[c];
                    prev_row[c] = 1 + prev.min(prev_row[c].min(prev_row[c - 1]));
                    max_sz = max_sz.max(prev_row[c]);
                    prev = tmp;
                }
            }
        }
    ...
    pub fn max_path_sum(root: Option<Rc<RefCell<TreeNode>>>) -> i32 {
        fn dfs(node: Option<Rc<RefCell<TreeNode>>>, curr_max: &mut i32) -> i32 {
            if let Some(some) = node {
                let left_sum = 0.max(dfs(RefCell::borrow_mut(&some).left.clone(), curr_max));
                let right_sum = 0.max(dfs(RefCell::borrow_mut(&some).right.clone(), curr_max));
                let val = RefCell::borrow_mut(&some).val;
                *curr_max = (*curr_max).max(left_sum + right_sum + val);
