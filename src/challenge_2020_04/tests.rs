...
fn get_single_number_test_cases() -> Vec<(Vec<i32>, i32)> {
    vec![(vec![2, 2, 1], 1), (vec![4, 1, 2, 1, 2], 4)]
}
...
fn group_anagrams_v2_test() {
    assert_eq!(
        Solution::group_anagrams_v2(vec![]),
        Vec::<Vec<String>>::new()
    );
    assert_eq!(
        Solution::group_anagrams_v2(vec![
...
fn list_helpers_test() {
    ...
    assert_eq!(
        ListNode::from_slice(&[1, 2, 3, 4, 5]).to_vec(),
        vec![1, 2, 3, 4, 5]
    );
}
...
fn middle_node_test() {
    ...
    for case in test_cases {
        assert_eq!(
            Solution::middle_node(ListNode::from_slice(&case.0)).to_vec(),
            case.1
        );
    }
}
...
fn find_max_length_v2_test() {
    ...
    assert_eq!(
        Solution::find_max_length_v2(vec![0, 1, 1, 0, 1, 1, 1, 0]),
        4
    );
    assert_eq!(
        Solution::find_max_length_v2(vec![0, 0, 1, 0, 0, 0, 1, 1]),
        6
    );
}
...
fn product_except_self_test() {
    ...
    assert_eq!(
        Solution::product_except_self(vec![2, 3, 4, 5]),
        vec![60, 40, 30, 24]
    );
}
...
fn product_except_self_v2_test() {
    ...
    assert_eq!(
        Solution::product_except_self_v2(vec![1, 2, 3, 4]),
        vec![24, 12, 8, 6]
    );
    assert_eq!(
        Solution::product_except_self_v2(vec![2, 3, 4, 5]),
        vec![60, 40, 30, 24]
    );
}
...
fn check_valid_string_test() {
    ...
    assert_eq!(Solution::check_valid_string("**)**".to_string()), true);
    assert_eq!(
        Solution::check_valid_string("(((******))".to_string()),
        true
    );
    assert_eq!(Solution::check_valid_string("(*))".to_string()), true);
    assert_eq!(Solution::check_valid_string("(*)())".to_string()), true);
...
fn bst_from_preorder_test() {
    for case in get_bst_from_preorder_test_cases() {
        assert_eq!(
            Solution::bst_from_preorder(case.0).get_level_order_values(),
            case.1
        );
    }
}
...
fn bst_from_preorder_v2_test() {
    for case in get_bst_from_preorder_test_cases() {
        assert_eq!(
            Solution::bst_from_preorder_v2(case.0).get_level_order_values(),
            case.1
        );
    }
}
...
fn longest_common_subsequence() {
    assert_eq!(
        Solution::longest_common_subsequence("".to_string(), "a".to_string()),
        0
    );
    assert_eq!(
    ...
    assert_eq!(
        Solution::longest_common_subsequence("aggtab".to_string(), "gxtxayb".to_string()),
        4
    );
}
...
fn maximal_square_v2_test() {
    ...
    assert_eq!(Solution::maximal_square_v2(vec![vec!['0']]), 0);
    assert_eq!(Solution::maximal_square_v2(vec![vec!['1']]), 1);
    assert_eq!(
        Solution::maximal_square_v2(vec![
            vec!['1', '0', '1', '0', '0'],
            vec!['1', '0', '1', '1', '1'],
            vec!['1', '1', '1', '1', '1'],
            vec!['1', '0', '0', '1', '0'],
...
fn max_path_sum_test() {
    ...
    for case in test_cases {
        assert_eq!(
            Solution::max_path_sum(TreeNode::from_level_order(&case.0)),
            case.1
        );
    }
}
...
fn is_valid_sequence_test() {
    ...
    for case in test_cases {
        assert_eq!(
            Solution::is_valid_sequence(TreeNode::from_level_order(&case.0), case.1),
            case.2
        );
    }
}
